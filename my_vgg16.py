#本函数由tensorflow\python\keras\application\vgg16.py修改而来，主要修改内容：
#1. conv block5 层的pooling去掉
#
#本函数输入参数只能设置为VGG16(include_top=False,weights=non_top参数路径)，其他输入参数设置将造成错误
#本函数返回一个以VGG网络中， conv block5为输出（不含block5的pooling）的，加载已训练好的weights,参数不可训练的model
